...
This keeps the large legacy handler.py mostly unchanged while adding:
...
_ORIGINAL_QWEN_PROMPT = h.qwen_prompt
...
def process_album_events_with_local_image_text(job_id: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    should_run_embeddings = bool(
        steps.get("embeddings")
        or payload.get("run_embeddings")
    )
    ...
    if should_run_embeddings:
        h.update_job_status(
            ...
            "running",
            "embeddings",
            "Generating text embeddings from image-text search_text",
        )
